  ...
  Future<void> _loadReviews() async {
    try {
      final res = await http.get(
          Uri.parse('${AppConstants.apiBaseUrl}${ApiEndpoints.reviews}'));
      if (res.statusCode == 200) {
        final body = jsonDecode(res.body);
        final List list = (body is Map && body['data'] is List) ? body['data'] : [];
        final items = <_Review>[];
        for (final e in list) {
          final name = (e['customerName'] ?? 'Guest').toString();
          final rating = (e['rating'] is num) ? (e['rating'] as num).toInt() : 5;
          final text = (e['comment'] ?? '').toString();
          if (text.isNotEmpty) items.add(_Review(name, rating, text));
        }
  ...
  Widget build(BuildContext context) {
    ...
    return CustomScrollView(
      slivers: [
        ...
        // ── Category chips ───────────────────────────────────────────────────
        SliverAppBar(pinned: true, primary: false, toolbarHeight: 42, automaticallyImplyLeading: false, backgroundColor: AppColors.primary, titleSpacing: 0, title: const _OfferBar()),
        SliverToBoxAdapter(
          child: SizedBox(
            height: 56,
...
class _FeaturedBannerState extends State<_FeaturedBanner> {
  String _img = '';
  String _eyebrow = 'Bridal Special';
  String _title = 'Complete Bridal Package';
  String _subtitle = 'Make your special day unforgettable';
  bool _active = true;
  ...
  Future<void> _loadBanner() async {
    try {
      final res =
          await http.get(Uri.parse('${AppConstants.apiBaseUrl}/api/banner'));
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body)['data'];
        if (data != null && mounted) {
          setState(() {
            _img = (data['imageUrl'] ?? '').toString();
            final e = (data['eyebrow'] ?? '').toString();
            final t = (data['title'] ?? '').toString();
            final sub = (data['subtitle'] ?? '').toString();
            if (e.isNotEmpty) _eyebrow = e;
            if (t.isNotEmpty) _title = t;
            if (sub.isNotEmpty) _subtitle = sub;
...
class _HomeImg {
  final String url;
  final String title;
  const _HomeImg(this.url, this.title);
}
...
class _GallerySectionState extends State<_GallerySection> {
  ...
  Future<void> _loadHomeImages() async {
    try {
      final res = await http.get(Uri.parse('${AppConstants.apiBaseUrl}/api/home-images'));
      if (res.statusCode == 200) {
        final body = jsonDecode(res.body);
        final List list = (body is Map && body['data'] is List) ? body['data'] : [];
        final items = <_HomeImg>[];
        for (final e in list) {
          final url = (e['url'] ?? '').toString();
          if (url.isNotEmpty) items.add(_HomeImg(url, (e['title'] ?? '').toString()));
        }
            ...
            return ClipRRect(
              ...
              child: SizedBox(
                ...
                child: Stack(
                  fit: StackFit.expand,
                  children: [
                    Image.network(img.url, fit: BoxFit.cover, alignment: Alignment.topCenter,
...
// Offer / announcement scroller (text will come from SaaS panel later)
const _kOfferText =
    'Eid Special: 20% off all bridal packages this week - book now and glow!        Walk-ins welcome, 11 AM to 9 PM daily.';

class _OfferBar extends StatefulWidget {
  const _OfferBar();
  @override
  State<_OfferBar> createState() => _OfferBarState();
}

class _OfferBarState extends State<_OfferBar> {
  String _text = _kOfferText;
  bool _active = true;
  ...
  Future<void> _loadAnnouncement() async {
    try {
      final res = await http.get(
          Uri.parse('${AppConstants.apiBaseUrl}/api/announcement'));
      if (res.statusCode == 200) {
        final body = jsonDecode(res.body);
        final data = (body is Map) ? body['data'] : null;
        if (data is Map) {
          final t = (data['text'] ?? '').toString().trim();
          final a = data['isActive'] != false;
          if (mounted) setState(() { _active = a; if (t.isNotEmpty) _text = t; });
        }
      }
    } catch (_) {}
  }
